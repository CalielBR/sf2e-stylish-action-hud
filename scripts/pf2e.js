...
export class Pf2eAdapter extends BaseSystemAdapter {
	...
	getStats(actor, config) {
		...
		return config.map((attr) => {
			...
			// PF2e Perception — Statistic 객체이므로 별도 처리
			if (attr.path === "system.attributes.perception.value") {
				...
				if (typeof prop === "number") {
					// 1. 그냥 숫자인 경우 (예: attributes.ac.value)
					val = prop;

					// 특수 케이스: AC 같은 경우 Max가 없지만 0으로 나누면 안됨
					// 경로가 .value로 끝나면 .max를 한번 찾아봄
					if (attr.path.endsWith(".value")) {
						const maxPath = attr.path.replace(".value", ".max");
						max = foundry.utils.getProperty(actor, maxPath) ?? 0;
					}
				} else if (typeof prop === "object" && prop !== null) {
					// 2. 객체인 경우 (예: attributes.hp -> {value, max, temp})
					val = prop.value ?? 0;
					max = prop.max ?? 0;
					temp = prop.temp ?? 0;
				}
				...
				// 특수 보정: 쉴드(Shield)
				// 쉴드가 없거나 깨졌으면 max가 0일 수 있음
				if (attr.path.includes("shield")) {
					// PF2e에서 쉴드 데이터는 attributes.shield 에 있음
			...
			if (max > 0) {
				...
				// Max가 없는 수치(AC, 수량 등)는 값이 있으면 100%로 보여줌
				percent = 100;
			}
	...
	async _getSystemSubMenuData(actor, systemId, menuData) {
		switch (systemId) {
			...
			case "feat":
				return { ...this._getFeatData(actor), title: menuData.label };
			case "inventory":
					...
					if (isFocusEntry) {
						...
						for (const group of sheetData.groups) {
							...
							if (rankSpells.length > 0) {
								// 네이티브 주문 먼저, virtual(heightened) 주문은 아래로
								rankSpells.sort((a, b) => {
		...
		if (game.pf2e?.actions) {
			game.pf2e.actions.forEach(action => {
				...
				if (basicSlugs.includes(action.slug)) {
					...
					// Apenas ações que o sistema mapeia como Skill Actions
	...
	_getFeatData(actor) {
		...
		// 카테고리 초기화
		const categories = {
			action: { label: game.i18n.localize("IBHUD.SubCategory.Actions"), items: [] },
			reaction: { label: game.i18n.localize("IBHUD.SubCategory.Reactions"), items: [] },
			free: { label: game.i18n.localize("IBHUD.SubCategory.FreeActions"), items: [] },
		};
		...
		// 1. Actions
		items["action"] = { all: categories.action.items };
		subLabels["action"] = { all: game.i18n.localize("IBHUD.SubCategory.Actions") };

		// 2. Reactions
		items["reaction"] = { all: categories.reaction.items };
		subLabels["reaction"] = { all: game.i18n.localize("IBHUD.SubCategory.Reactions") };

		// 3. Free Actions
		items["free"] = { all: categories.free.items };
		subLabels["free"] = { all: game.i18n.localize("IBHUD.SubCategory.FreeActions") };
	...
	_getUtilityData(actor) {
		...
		if (percStat) {
			...
			items["skill"]["all"].unshift({
				...
				cost: `${mod >= 0 ? "+" : ""}${mod}`,
				description: "Roll Perception",
			});
		}

		// 3. 기타 (Initiative, Rest)
		...
		if (init) {
			// 이니셔티브는 보통 Perception 기반이지만 스킬일 수도 있음.
			// actor.initiative.mod에 현재 설정된 이니셔티브 수정치가 들어있음.
		...
		const primaryTooltips = {};
		Object.keys(categories).forEach((key) => {
			// ★ [수정] 매크로 탭은 내용물이 (안내문구 포함) 무조건 있으므로 숨기지 않음
	...
	async _handleItem(actor, itemId, event, context = {}) {
		...
		if (item.type === "spell") {
			...
			if (entry && entry.cast) {
				// ★ [핵심] 준비된 주문(Prepared)의 경우, 특정 슬롯(slotIndex)을 지정해야 함
	...
	_getActionGlyph(cost) {
		...
		// 데이터가 "1" (String) 혹은 1 (Number)일 수 있음
		...
		const map = {
			...
			reaction: "R",
			free: "F",
			action: "1", // 가끔 action으로 적힌 경우도 있음
		};
	...
	/**
	 * [신규] 인벤토리에 없는 가상 아이템(Unarmed Strike 등)을 찾는 헬퍼
	 */
	...
	// ★ [헬퍼] PF2e의 자동 비밀굴림(Secret) 감지 방지용 이벤트 세탁기
	_cleanEvent(e) {
		if (!e) return {};
		return {
			shiftKey: e.shiftKey,
